...
VALUES ('quotesapi', 'API that exposes popular quotes', '/quotesapi');
...
FROM db, api
WHERE db.designation = 'quotesdb' AND api.designation = 'quotesapi';
...
INSERT INTO attribute (collection_id, attributename, columnname, type, format)
SELECT id, 'name', 'author_id', 'integer', 'int32'
FROM collection WHERE  tablename='author';
...
SELECT id, 'author_name', 'author_name', 'string'
FROM collection WHERE  tablename='v_quote';
