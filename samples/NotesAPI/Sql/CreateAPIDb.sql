...
'notesdb', id, 'Data Source=notesdb.sqlite'
FROM dbtype
...
FROM collection WHERE tablename='note';

INSERT INTO attribute (collection_id, attributename, columnname, description, type, format, fiqlkeyindex)
SELECT id, 'title', 'title', 'The title of the note', 'string', NULL, 2
FROM collection WHERE  tablename='note';

INSERT INTO attribute (collection_id, attributename, columnname, description, type, format, fiqlkeyindex)
SELECT id, 'content', 'content', 'The content of the note. It is recommanded to exclude the title from this data !', 'string', NULL, 3
FROM collection WHERE  tablename='note';
...
INSERT INTO attribute (collection_id, attributename, columnname, type, format, autovalue, keyindex, fiqlkeyindex)
SELECT id, 'id', 'id', 'integer', 'int32', 1, 0, 1
FROM collection WHERE tablename='picture';

INSERT INTO attribute (collection_id, attributename, columnname, type, format, fiqlkeyindex)
SELECT id, 'note_id', 'note_id', 'integer', 'int32', 2
FROM collection WHERE tablename='picture';

INSERT INTO attribute (collection_id, attributename, columnname, description, type, format, fiqlkeyindex)
SELECT id, 'title', 'title', 'The title of the picture', 'string', NULL, 3
FROM collection WHERE  tablename='picture';

INSERT INTO attribute (collection_id, attributename, columnname, description, type, format)
SELECT id, 'data', 'data', 'The data of the picture', 'string', 'binary'
FROM collection WHERE  tablename='picture';